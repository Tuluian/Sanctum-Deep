...
- [x] Design The Bonelord boss (AC: 1, 2, 3, 4)
  - [x] **The Bonelord** (150 HP)
...
- [x] Implement boss-specific mechanics
  - [x] 3-phase system with visual transitions
  - [x] Charge-up attacks (telegraph for X turns, then execute)
  - [x] Resurrect dead minions ability
  - [x] "All minions attack" command ability
  - [x] Bound status effect (cannot play Attack cards)

- [x] Implement charge-up system
  - [x] Enemy enters "charging" state with countdown
  - [x] Show charge intent with turn counter (⚡ Charging...)
  - [x] Execute powerful attack when charge completes
  - [x] Player can try to kill boss before charge finishes

- [x] Implement minion command system
  - [x] Dark Command triggers all minion attacks immediately
  - [x] Minions use their basic attack when commanded
  - [x] Works with any summoned enemies

- [x] Update UI for boss fight
  - [x] Boss health bar (larger, more prominent)
  - [x] Phase indicator (Phase 2, Phase 3)
  - [x] Charge-up counter display with pulsing animation
  - [x] Boss name plate with skull icon
  - [ ] Screen effects on phase transition (deferred - optional polish)

- [ ] Implement boss rewards (deferred to rewards system story)
  - [ ] Large gold reward (100+ gold)
  - [ ] Choice of rare card
  - [ ] Boss relic (unique item, one per boss)
  - [ ] Act 2 unlock

- [ ] Design Bonelord's Relic (deferred to relics system story)
  - [ ] **Skull of the Bonelord**: Start each combat with 5 block. Gain 1 block whenever an enemy dies.

- [x] Write unit tests
  - [x] Test boss has 150 HP
  - [x] Test phase transitions at 66% and 33% HP
  - [x] Test charge-up countdown works correctly
  - [x] Test charged attack deals 30 damage
  - [x] Test Dark Command triggers minion attacks
  - [x] Test Raise Dead resurrects fallen minions
  - [x] Test Bound prevents playing Attack cards
...
- Implemented The Bonelord boss with 150 HP and 3 phases
- Created Skeleton (20 HP) and Bone Archer (15 HP) minion definitions
- Added IntentType.CHARGING and IntentType.COMMAND to types
- Implemented ChargingState interface for charge-up attacks
- Added charging mechanic to CombatEngine (starts charging, decrements, executes)
- Added Dark Command mechanic (all minions attack immediately)
- Added minion resurrection for Raise Dead ability
- Added Bound status effect that prevents playing Attack cards
- Added status effect duration decrement at end of turn
- Created boss-specific CSS styling (larger figure, purple glow, phase indicator, charging animation)
- Created 23 unit tests covering all boss mechanics
- Fixed existing tests to account for duration decrement behavior

### File List
- `src/data/enemies/bosses.ts` (NEW) - Boss and minion definitions
- `src/data/enemies/bosses.test.ts` (NEW) - 23 unit tests
- `src/engine/CombatEngine.ts` (MODIFIED) - Boss mechanics, charging, command, bound
- `src/types/index.ts` (MODIFIED) - ChargingState, isBoss, new IntentTypes
- `src/ui/renderer.ts` (MODIFIED) - Boss display, phase indicator, charging intent
- `src/styles/main.css` (MODIFIED) - Boss styling, charging animation
- `src/data/enemies/act1.test.ts` (MODIFIED) - Fixed duration expectation
- `src/data/enemies/elites.test.ts` (MODIFIED) - Fixed duration expectations
