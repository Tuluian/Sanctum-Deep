...
These enemies are not the freshly-fallen. They're souls who lasted—who made compromises to survive, who bargained with the Hollow God, who lost themselves slowly instead of all at once.
...
**Death Quote:** *[Multiple overlapping voices]* "We're... separate now? I... I remember my name. It was—" *[Silence. Too late.]*

**Visual Notes:** Faces occasionally surface in the mass—not horrified, but confused. Reaching toward something they can't remember.
...
**Death Quote:** *"I had a name. I had a family. I had—"* *[Absorbed back into nothing.]*
...
**What They Were:** Wizards who entered seeking knowledge of the void. The Hollow God taught them—and teaching is consuming here. They can still cast, but the spells taste like ash.

**Death Quote:** *"I learned too much. I learned what's behind everything. It's... nothing. It was always nothing."*

**Visual Notes:** Robes are pristine but they're fading—becoming translucent. You can see the void behind them.
...
**What It Was:** Not a person—a construct created by the Drowned King to guard his crypts. When the King fell, the Gargoyles lost their purpose. They guard... everything. They attack... everyone.
...
**What They Were:** A soul that died but didn't leave. They wanted revenge on something—the Sanctum, the Hollow God, themselves—and that want kept them here. Now they don't remember what they wanted. They just... hurt.
...
**Visual Notes:** Translucent, flickering. When they attack, you see a flash of who they were—then it's gone.
...
**What They Were:** Two souls, merged wrong. When the Hollow God tried to consume two delvers who were holding hands—lovers, siblings, friends—it couldn't separate them. Now they're neither person. They're a mistake.
...
**What They Were:** Originally a fisher from the coast near Thalassar. When the kingdom flooded, they tried to save the children. The water changed them. Now they are children of the deep—and they remember nothing of the sun.
...
**Visual Notes:** Fish-like features, scales, gills. Still wearing scraps of a fisherman's clothes. A child's bracelet on one wrist—fused to the skin.
...
- [x] Implement enemy definitions (AC: 2, 3)
  - [x] Create `src/data/enemies/act2.ts`
  - [x] Add all 8 enemies with proper move definitions
  - [x] Define moves with proper intent types
  - [x] Set appropriate weights for move selection

- [x] Implement new Act 2 mechanics (AC: 7) - **Partial: Data definitions only**
  - [x] **Corrupt** status: Added to StatusType enum with REGENERATION
  - [x] **Spawn** mechanic: Added IntentType.SPAWN and spawnId/hpThreshold to EnemyMove
  - [ ] **Magic Shield**: Deferred - using regular block for now
  - [ ] **Conditional moves**: Gargoyle's Awakening condition - deferred to CombatEngine update

- [ ] Update CombatEngine for new mechanics - **Future story**
...
- [x] Update UI for new mechanics (AC: 3)
  - [x] Corrupt status icon (🩻)
  - [x] Regeneration status icon (💚)
  - [ ] Spawn animation - deferred to CombatEngine update

- [ ] Create encounter configurations - **Future story**
  - [ ] Define which enemies can appear together
  - [ ] Define encounter difficulty tiers

- [x] Write unit tests
  - [x] Test each enemy has correct HP and moves (40 tests)
  - [x] Test Slime has split move with hpThreshold
  - [x] Test Corrupt debuff is defined correctly
  - [x] Test move weights sum to 100
  - [x] Test enemy retrieval functions
...
> "The deeper chambers hold those who survived longer before failing — dark mages who bargained with the Hollow God, aberrations formed from merged souls, gargoyles that were once stone guardians driven mad."
...
- Build error: Missing CORRUPT and REGENERATION in renderer.ts STATUS_INFO - Fixed by adding entries

### Completion Notes List
- Created 8 Act 2 enemy definitions with thematic moves matching "Sunken Crypts" narrative
- Added CORRUPT and REGENERATION to StatusType enum
- Added SPAWN to IntentType enum for Slime splitting mechanic
- Extended EnemyMove interface with debuffAmount, spawnId, hpThreshold fields
- Created 40 unit tests covering all enemies and helper functions
- Added status icons to renderer.ts for new status types
- CombatEngine changes for SPAWN/CORRUPT mechanics deferred to future story

### File List
- `src/types/index.ts` - Added CORRUPT, REGENERATION StatusTypes; SPAWN IntentType; extended EnemyMove interface
- `src/data/enemies/act2.ts` - NEW: 8 Act 2 enemy definitions with ACT2_ENEMIES export
- `src/data/enemies/act2.test.ts` - NEW: 40 unit tests for Act 2 enemies
- `src/ui/renderer.ts` - Added CORRUPT and REGENERATION to STATUS_INFO
