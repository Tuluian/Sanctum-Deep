...
- [x] Design Void Caller elite (AC: 1, 2, 3)
  - [x] **Void Caller** (85 HP) - Summoner who bargained with the Hollow God
    - **Phase 1** (HP > 50%): Summons and buffs
      - Summon Void Tendril (Spawn a 20 HP minion, 35%)
      - Void Bolt (Attack 10, 35%)
      - Empower Void (All Void Tendrils gain +3 Might, 30%)
    - **Phase 2** (HP <= 50%): Desperate attacks
      - Void Storm (Attack 6, 40%)
      - Consume Tendril (Heal 15 HP, 30%)
      - Final Bargain (Attack 15, apply Cursed, 30%)

- [x] Design Void Tendril minion (AC: 1)
  - [x] **Void Tendril** (20 HP) - Spawned by Void Caller
    - Lash (Attack 5, 60%)
    - Entangle (Apply Bound 1 turn, 40%)

- [x] Design Stone Sentinel elite (AC: 1, 2, 3)
  - [x] **Stone Sentinel** (100 HP) - Ancient guardian driven mad
    - **Normal Moves** (HP > 30%):
      - Crushing Blow (Attack 12, 30%)
      - Stone Reformation (Block 15, 25%)
      - Tremor (Attack 7, 25%)
      - Petrifying Gaze (Apply Bound 1 turn, 20%)
    - **Enrage** (HP <= 30%):
      - Crushing Blow Enraged (Attack 17 = 12+5, 40%)
      - Tremor Enraged (Attack 12 = 7+5, 35%)
      - Final Guard (Attack 10, Block 8, 25%)

- [x] Implement elite-specific mechanics - **Data definitions only**
  - [x] Multi-phase behavior based on HP thresholds (phaseThresholds array)
  - [x] Spawn intent for Void Caller (IntentType.SPAWN)
  - [x] Enrage state through separate phase with +5 damage moves
  - [ ] Stone Armor mechanic - deferred to CombatEngine update
  - [ ] "Hits all" attacks - deferred to CombatEngine update

- [ ] Update CombatEngine for elite mechanics - **Future story**
  - [ ] Handle Stone Armor regeneration
  - [ ] Handle "damage all" effects (enemies and player)
  - [ ] Handle consume tendril mechanic

- [ ] Update UI for elite encounters - **Future story**
  - [ ] Elite health bar (larger, golden border)
  - [ ] Phase indicator
  - [ ] Enrage visual effect (red glow)
  - [ ] Lore text display before combat

- [ ] Implement elite rewards - **Future story**
  - [ ] Guaranteed uncommon or rare card choice
  - [ ] +50% gold compared to normal combat

- [x] Write unit tests (42 tests)
  - [x] Test Void Caller phase transition at 50% HP
  - [x] Test Void Caller can spawn void tendrils
  - [x] Test Stone Sentinel enrage at 30% HP
  - [x] Test enrage increases damage by +5
  - [x] Test elite HP ranges (80-100)
...
- Created Void Caller (85 HP) with 2-phase behavior (summoner → desperate)
- Created Void Tendril (20 HP) as spawnable minion with Lash/Entangle moves
- Created Stone Sentinel (100 HP) with 2-phase behavior (normal → enraged at 30% HP)
- Enrage implemented via separate phase with +5 damage baked into move definitions
- All move weights sum to 100 per phase
- CombatEngine updates for "damage all", Stone Armor, consume mechanics deferred to future story
- 42 unit tests covering phases, HP thresholds, damage scaling, elite filtering

### File List
- `src/data/enemies/act2-elites.ts` - NEW: 3 enemy definitions (2 elites + 1 minion)
- `src/data/enemies/act2-elites.test.ts` - NEW: 42 unit tests for Act 2 elites
