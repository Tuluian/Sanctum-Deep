...
- [x] Set up UI rendering foundation
  - [x] Create UI component structure (DOM-based or Canvas)
  - [x] Implement `render()` function that updates all UI from game state
  - [x] Subscribe to combat engine state change events
  - [x] Re-render affected components on state change

- [x] Implement player stats display (AC: 1)
  - [x] HP display: "CurrentHP / MaxHP" with visual HP bar
  - [x] Block display: Shield icon with block value
  - [x] Resolve display: "Current / Max" (e.g., "2/3")
  - [x] Devotion display: Golden counter for Cleric mechanic
  - [x] Update stats immediately when they change

- [x] Implement enemy display area (AC: 2)
  - [x] Enemy name prominently displayed
  - [x] HP bar with percentage fill and "Current/Max" text
  - [x] Block indicator (shield icon) when enemy has block
  - [x] Intent display showing next action with icon + value
    - Attack: Sword icon + damage number
    - Defend: Shield icon + block number
  - [x] Support multiple enemies in a row/grid

- [x] Implement hand display (AC: 3, 4)
  - [x] Card component showing: Name, Cost (in corner), Type label, Description
  - [x] Cards arranged horizontally with overflow handling
  - [x] Attack cards: Red border/accent
  - [x] Skill cards: Blue border/accent
  - [x] Playable cards: Full opacity, hover effect
  - [x] Unplayable cards: Reduced opacity (50%), no hover effect

- [x] Implement card interaction (AC: 5)
  - [x] Click/tap on playable card initiates play
...
- [x] Implement End Turn button (AC: 6)
  - [x] Large, prominent button below hand area
  - [x] "END TURN" text clearly visible
  - [ ] Disabled state during enemy turn
  - [ ] Visual feedback on click (button press animation)
  - [x] Triggers combat engine endTurn()

- [x] Implement deck pile displays (AC: 7)
  - [x] Draw pile: Card stack icon + count number
  - [x] Discard pile: Card stack icon + count number
  - [x] Position at bottom corners or near hand area
  - [x] Update counts when cards are drawn/discarded

- [x] Implement combat log (AC: 8)
  - [x] Scrollable log area (max height with overflow)
  - [x] Show recent combat events:
    - "You dealt X damage to [Enemy]"
    - "You gained X block"
    - "You healed X HP"
    - "[Enemy] dealt X damage (Y blocked)"
    - "[Enemy] gained X block"
  - [x] New entries appear at bottom, auto-scroll
  - [ ] Limit to last 20-30 entries

- [x] Implement game over screens (AC: 9)
  - [x] Victory screen: "VICTORY!" message, green/gold theme
  - [x] Defeat screen: "DEFEAT" message, red/dark theme
  - [x] Overlay modal that covers combat area
  - [x] "New Combat" or "Restart" button
  - [ ] Basic stats display (turns taken, damage dealt, etc.)

- [x] Implement dead enemy visual state (AC: 2)
  - [x] Dead enemies: Reduced opacity, grayed out
  - [x] Dead enemies: Not targetable, cursor shows "not allowed"
...
- Attack: "⚔️" or "Attacking for X"
- Defend: "🛡️" or "Defending for X"
- Unknown: "❓"
...
- DOM-based UI using CombatRenderer class
- Player stats, enemy display, hand, combat log all implemented
- Game over modal with restart button works
- All 9 core acceptance criteria are met
- Polish items deferred to future iterations:
  - Proper target selection UX (currently uses pre-selected enemy)
  - Mobile responsiveness testing
  - Animations and visual feedback

### File List
- src/ui/renderer.ts (created)
- src/main.ts (Game class orchestration)
