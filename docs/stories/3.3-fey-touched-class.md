...
- [ ] Create Fey-Touched class configuration (AC: 1, 2)
  - [ ] Define FEY_TOUCHED class with 65 HP, 3 Resolve
  - [ ] Mark as premium/DLC class
  - [ ] Define starter deck recipe

- [ ] Implement Fey-Touched starter deck cards (AC: 3)
  - [ ] **Fey Bolt**: { id: 'fey_bolt', type: ATTACK, cost: 1, whimsy: [DAMAGE 4, DAMAGE 5, DAMAGE 6, DAMAGE 7, DAMAGE 8] }
  - [ ] **Glamour**: { id: 'glamour', type: SKILL, cost: 1, whimsy: [BLOCK 4, BLOCK 5, BLOCK 6, BLOCK 7, BLOCK 8] }
  - [ ] **Pixie's Gift**: { id: 'pixies_gift', type: SKILL, cost: 1, whimsy: [DRAW 2, BLOCK 8, DAMAGE 8] }
  - [ ] **Trickster's Trade**: { id: 'tricksters_trade', type: SKILL, cost: 1, effects: [TRANSFORM_RANDOM_CARD] }

- [ ] Create fey-touched cards data file
  - [ ] Create `src/data/cards/fey_touched.ts`
  - [ ] Export FEY_TOUCHED_CARDS record
  - [ ] Register in card index

- [ ] Implement Whimsy mechanic (AC: 4, 5)
  - [ ] Add `whimsy` property to CardDefinition (array of possible effects)
  - [ ] On play, randomly select one outcome
  - [ ] Display all possible outcomes on card hover
  - [ ] Show selected outcome during resolution
  - [ ] Emit WHIMSY_RESOLVED event with chosen outcome

- [ ] Implement random range effects
  - [ ] DAMAGE with min/max range
  - [ ] BLOCK with min/max range
  - [ ] Uniform random distribution within range

- [ ] Implement card transformation (AC: 6)
  - [ ] TRANSFORM_RANDOM_CARD: Replace random card in hand
  - [ ] Transformation can create any card of same rarity
  - [ ] TRANSFORM_THIS_CARD: Card transforms after use
  - [ ] Track transformed cards for UI

- [ ] Implement Luck resource (AC: 7)
  - [ ] Add `luck` to PlayerState (0-10 range)
  - [ ] Gain Luck from certain cards/events
  - [ ] Spend Luck to:
    - Reroll Whimsy outcome (costs 3 Luck)
    - Guarantee high roll (costs 5 Luck)
    - Choose Whimsy outcome (costs 7 Luck)
  - [ ] Luck persists across turns, resets each combat
  - [ ] Display Luck in UI

- [ ] Implement Whimsy UI (AC: 5)
  - [ ] Show possible outcomes on card (e.g., "4-8 damage")
  - [ ] Hover tooltip lists all outcomes with probabilities
  - [ ] Animation when Whimsy resolves (slot machine effect?)
  - [ ] Highlight chosen outcome
  - [ ] Luck spend buttons when Whimsy triggers

- [ ] Implement seeded randomness
  - [ ] Whimsy uses combat RNG seed
  - [ ] Results are deterministic for replay/sharing
  - [ ] Each Whimsy advances RNG state

- [ ] Write unit tests
  - [ ] Test Fey-Touched starts with 65 HP
  - [ ] Test Fey Bolt deals between 4-8 damage
  - [ ] Test Pixie's Gift selects one of three outcomes
  - [ ] Test Trickster's Trade transforms a card
  - [ ] Test Luck accumulates correctly
  - [ ] Test Luck spend rerolls outcome
  - [ ] Test RNG is seeded/deterministic
