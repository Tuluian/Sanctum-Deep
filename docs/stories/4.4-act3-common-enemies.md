...
### File List
- `src/data/enemies/act3.ts` - NEW: 6 Act 3 enemy definitions with ACT3_ENEMIES export
- `src/data/enemies/act3.test.ts` - NEW: 44 unit tests for Act 3 enemies
- `src/types/index.ts` - MODIFIED: Added INTANGIBLE to StatusType, added intangible property to Enemy interface
- `src/engine/CombatEngine.ts` - MODIFIED: Added Act 3 mechanics (Intangible, Consume Light, Purge, Judgment, Fade, Giggle, Howl)
- `src/ui/renderer.ts` - MODIFIED: Added INTANGIBLE status display info
