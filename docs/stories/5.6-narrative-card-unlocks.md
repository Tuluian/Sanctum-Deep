...
- [ ] Implement Narrative Event System
  - [ ] Create `src/types/narrativeEvents.ts` (use interfaces from `narrative-events-complete.md`)
  - [ ] Create `src/data/narrativeEvents/` directory structure
  - [ ] Create `src/data/narrativeEvents/universal.ts` - Universal events
  - [ ] Create `src/data/narrativeEvents/classSpecific.ts` - Class-specific events
  - [ ] Create `src/services/NarrativeEventService.ts`

- [ ] Implement Event Trigger System
  - [ ] Add event trigger checks after room completion
  - [ ] Implement progress-based triggers (rooms cleared)
  - [ ] Implement health-based triggers (HP thresholds)
  - [ ] Implement boss-related triggers (pre/post boss)
  - [ ] Implement random triggers (% chance per room)
  - [ ] Implement class restriction checks

- [ ] Implement Event Choice/Outcome System
  - [ ] Create weighted random outcome selection
  - [ ] Implement reward application (HP, Resolve, Damage, Cards)
  - [ ] Implement penalty application (HP loss, Curses)
  - [ ] Implement special outcomes (skip encounter, start combat)
  - [ ] Track choices for unlock triggers

- [ ] Implement Event UI
  - [ ] Create `src/ui/screens/NarrativeEventScreen.ts`
  - [ ] Full-screen parchment overlay
  - [ ] Speaker portrait (Warden, Character, Unknown)
  - [ ] Choice buttons with hover flavor text
...
- [ ] Create event-to-unlock mappings
  - [ ] Create `src/data/eventUnlockMappings.ts`
  - [ ] Map all events from `narrative-events-complete.md` to card unlocks
  - [ ] Integrate with NarrativeEventService
...
- [ ] Define unlockable cards
  ...
  - [ ] Define 15+ event-unlock cards (from narrative events)
  - [ ] Add unlock requirements to each

### Phase 3: Integration & Polish

- [ ] Integrate with game systems
  - [ ] Hook boss defeat events → CardUnlockService
  - [ ] Hook narrative event choices → CardUnlockService
  - [ ] Hook shrine choice events → CardUnlockService
  - [ ] Hook achievement triggers → CardUnlockService
...
- [ ] Write unit tests
  ...
  - [ ] Test weighted outcome selection
  - [ ] Test reward/penalty application
  - [ ] Test unlock trigger detection
  - [ ] Test persistence
